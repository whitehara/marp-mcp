...
export const chartBarLayout: SlideLayout = {
  name: "chart-bar",
  description: "Dark CSS horizontal bar chart for data visualization (use 'Label|Value' format)",
  params: {
    heading: {
      type: "string",
  ...
  template: (params) => {
    ...
    const barItems = parsed
      .map((b) => {
        ...
        return `<div class="dk-chart-bar-row">\n<span class="dk-chart-bar-label">${b.label}</span>\n<div class="dk-chart-bar-track"><div class="dk-chart-bar-fill" style="width:${pct}%"></div></div>\n<span class="dk-chart-bar-value">${b.value}</span>\n</div>`;
      })
      .join("\n");
